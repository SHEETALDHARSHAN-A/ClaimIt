...
from .models import UserModel, BkashPayment
from django.db import connection

def admin_login(request):
    try:
        # Fetch user from session based on email
        user = UserModel.objects.get(email=request.session.get('email'))
        
        # If POST request, handle login attempt
        if request.method == 'POST':
            ...
            if username and password:
                # Here you should verify the username and password against the database or hardcoded credentials
                if username == 'admin' and password == 'admin':  # You can modify this as needed
        ...
        messages.error(request, 'User not found, please login first.')
        return redirect('authenticate')
...
def admin_panel(request):
    try:
        ...
        if user.email == 'sheetaldharshan@gmail.com':
            # Query for pending payments
            pendings = BkashPayment.objects.raw(
                'SELECT * FROM bkash_payment WHERE STATUS = %s', ['Pending'])
            
            # Query for all users except the admin
            users = UserModel.objects.raw(
                'SELECT * FROM app_users WHERE email != %s', [request.session.get('email')])

            # Use raw SQL for claims
            cursor = connection.cursor()
            cursor.execute(
